...
has an impact factor, which is an integer. Nucleotides of types A, C, G and T have impact factors of 1, 2, 3 and 4,
respectively. You are going to answer several queries of the form: What is the minimal impact factor of nucleotides
...
public class GenomicRangeQuery {
    ...
    public static int[] solution(String S, int[] P, int[] Q) {
        ...
        for (int i = 0; i < P.length; i++) {
            ...
            for (int j = begin; j <= end; j++) {
                ...
                int currentFactor = getImpactFactor(nucleotides[j]);

                if (minFactor > currentFactor) {
    ...
    private static int[] getFactorsArray(String S) {

        char[] chars = S.toCharArray();
        int[] result = new int[chars.length];

        for (int i = 0; i < chars.length; i++) {
            result[i] = getImpactFactor(chars[i]);
        }
