...
Then only one fish can stay alive − the larger fish eats the smaller one. More precisely, we say that two fish P
and Q meet each other when P < Q, B[P] = 1 and B[Q] = 0, and there are no living fish between them. After they meet:
...
public class Fish {
    ...
    public int solution(int[] A, int[] B) {
        ...
        Deque<Integer> weight = new LinkedList<>();

        for (int i = 0; i < A.length; i++) {

            if (weight.isEmpty() && direction.isEmpty()) {
                weight.push(A[i]);
                direction.push(B[i]);
            } else {

                Integer prevDirection = direction.getFirst();

                    if (prevDirection == 1 && B[i] == 0) {

                        boolean win;

                        do {
                            int prevWeight = weight.getFirst();
                            if (prevWeight < A[i]) {
                                direction.pollFirst();
                                weight.pollFirst();
                                win = true;
                            ...
                            if (!direction.isEmpty())
                                prevDirection = direction.getFirst();
                            else
                                break;

                        } while (prevDirection == 1 && B[i] == 0);
